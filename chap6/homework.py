def homework(train_X, train_y, test_X):
    ...
    # Constructing calc graph and setting config og updating parameter
    ...
    # apply
    test = tf.placeholder(tf.float32, name='r')
    z1 = tf.nn.sigmoid(tf.matmul(test, layers[0].W) + layers[0].b)
    z2 = tf.nn.sigmoid(tf.matmul(z1, layers[1].W) + layers[1].b)
    z3 = tf.nn.softmax(tf.matmul(z2, layers[2].W) + layers[2].b)
    ans = tf.argmax(z3,1)
    pred_y = sess.run(ans, feed_dict={test: test_X})
    sess.close()
    return pred_y
